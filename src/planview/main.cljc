(ns planview.main
  "Kotodama WASM entrypoint for the Planview clean-room actor (L4) — Clojure port.
  ...
  facts (`planview.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "planview")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"planview.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "planview-compat" :tier tier :entities entities} 200])
...
(defn start! [] :planview-compat/ready)
